...
cleaned <- 
    articles %>%    
    filter(file %in% c("Kristoffer_extract_2.csv", "Zoli_extract_2.csv", "Lydia_extract_2.csv", "Tamas_extract_2.csv")) %>% 
    unnest() %>% 
...
counting_subs %>%
    ...
    mutate_at(vars(female:overweigh), funs(magrittr::multiply_by(., measurement_n))) %>% 
    # Calculate weighted proportions
    summarise_at(vars(female:overweigh),
                 funs(n = sum(., na.rm = TRUE), `%` = magrittr::divide_by(sum(., na.rm = TRUE), all_participants))) %>%
